...
"""
Trading Router - Order Execution & Trade Management  

Handles automated trading signal execution and trade lifecycle management.
...
@router.post(
    "/execute-signal", # Full path: /api/v1/trading/execute-signal
...
async def execute_trading_signal(
    ...
    """
    Execute a trading signal by placing a market order on KuCoin Futures.
    Automatically calculates position size, applies leverage, and logs the trade to database.
    
    - **signal**: Trading signal containing symbol, direction, entry/stop/target prices
    - **Returns**: Order execution details and database trade ID
    """
    logger.info(f"ExecuteSignal: Received for {signal.symbol}, Dir: {signal.direction.value}, "
                f"Strat: {signal.strategy_name}, Entry: {signal.entry_price}, SL: {signal.stop_loss}, "
                f"TP: {signal.take_profit}, Lev: {signal.suggested_leverage}")
    ...
    if signal.entry_price is None or signal.entry_price <= 0:
        logger.warning(f"ExecuteSignal: Entry price missing/invalid ({signal.entry_price}) for {signal.symbol}. "
                      f"Using trigger_price: {signal.trigger_price} as fallback.")
    ...
    if effective_entry_price is None or effective_entry_price <= 0:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
                          detail="Valid entry_price or trigger_price required in signal for amount calculation.")
    ...
    if not market_info:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, 
                          detail=f"Market info not found for symbol {ccxt_market_id}. "
                                f"Ensure it's a valid CCXT futures symbol.")
    ...
    if effective_entry_price > 0:
        ...
        if market_info.get('linear', True):  # Assume linear if 'linear' is True or not specified
            ...
            if contract_size > 0:
                ...
                raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
                                  detail="Invalid contract size for inverse contract.")
        else:
            logger.warning(f"ExecuteSignal: Contract type for {ccxt_market_id} is not explicitly linear or inverse. "
                          f"Assuming linear for amount calculation.")
            calculated_amount = position_value_usd / effective_entry_price
        ...
        if amount_precision is not None:
            ...
            logger.warning(f"ExecuteSignal: Amount precision not found for {ccxt_market_id}. "
                          f"Using raw calculated amount: {order_amount_in_base_currency}")

        if min_amount_limit is not None and order_amount_in_base_currency < min_amount_limit:
            logger.error(f"ExecuteSignal: Calculated order amount {order_amount_in_base_currency} for {ccxt_market_id} "
                        f"is below minimum exchange limit {min_amount_limit}.")
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
                              detail=f"Calculated order amount {order_amount_in_base_currency} is below minimum "
                                    f"{min_amount_limit} for {ccxt_market_id}.")
    else:
        logger.error(f"ExecuteSignal: Cannot calculate order amount due to invalid effective_entry_price "
                    f"({effective_entry_price}) for {ccxt_market_id}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
                          detail="Invalid effective_entry_price for amount calculation.")
    ...
    try:
        logger.info(f"ExecuteSignal: Placing {ccxt_side} {ccxt_order_type} order for {ccxt_market_id}, "
                    f"Amount: {order_amount_in_base_currency}, Leverage: {leverage_for_sizing}")
        ...
        if created_order_info and created_order_info.get('id'):
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
                              detail="Order placement failed (client error or no order ID).")
        ...
        if isinstance(original_ccxt_error, ccxt.InsufficientFunds):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
                              detail=f"Insufficient funds: {str(original_ccxt_error)}. Ensure sufficient balance and that "
                                    f"ISOLATED MARGIN LEVERAGE for {ccxt_market_id} is set correctly on KuCoin exchange "
                                    f"to match bot's intended leverage ({leverage_for_sizing}x).")
        elif isinstance(original_ccxt_error, ccxt.BadSymbol):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
                              detail=f"Invalid symbol '{ccxt_market_id}': {str(original_ccxt_error)}")
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
                              detail=f"KuCoin order structure error (SL/TP with market order): {str(original_ccxt_error)}")
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
                              detail=f"Margin/Leverage mode error: {str(original_ccxt_error)}. "
                                    f"Ensure symbol is configured for isolated margin with the correct leverage on KuCoin.")
