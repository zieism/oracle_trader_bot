...
class CoinbaseExchange(BaseExchange):
    """
    Coinbase Pro (Advanced Trade) exchange implementation using CCXT.
    """
    
    def __init__(self, api_key: str, api_secret: str, passphrase: str, testnet: bool = False, **kwargs):
    ...
    async def get_positions(self) -> List[Dict[str, Any]]:
        """Get open positions (Coinbase doesn't support futures, return empty)."""
        return []
    ...
    def normalize_symbol(self, symbol: str) -> str:
        ...
        """
        # Convert BTC/USDT to BTC-USD format for Coinbase
        if '/' in symbol:
            base, quote = symbol.split('/')
            # Coinbase primarily uses USD, not USDT
            if quote == 'USDT':
                quote = 'USD'
            return f"{base}-{quote}"
        return symbol
