...
class BinanceExchange(BaseExchange):
    ...
    Supports both spot and futures trading depending on configuration.
    """
    
    def __init__(self, api_key: str, api_secret: str, testnet: bool = False, futures: bool = False, **kwargs):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            raise ExchangeConnectionError(f"Failed to connect to Binance: {str(e)}")
    ...
    async def get_positions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for pos in positions:
                if pos['contracts'] and pos['contracts'] > 0:  # Only open positions
                    result.append({
                        "symbol": pos['symbol'],
                        "side": pos['side'],
    ...
    def normalize_symbol(self, symbol: str) -> str:
        ...
        Converts 'BTC/USDT' format to Binance format.
        """
        # For futures, might need to append suffix
        if self.futures and not symbol.endswith(':USDT'):
            if '/' in symbol:
